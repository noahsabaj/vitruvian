...
* ``SIGReg`` — projection-based stochastic isotropic-Gaussian regularizer.
* ``FeedForward`` / ``Attention`` / ``Block`` / ``ConditionalBlock`` —
  transformer building blocks used by ``Transformer`` and ``ARPredictor``.
* ``Transformer`` — LayerNorm-front, AdaLN-zero optional transformer stack.
* ``Embedder`` — 1D-conv action smoother → MLP embedder.
* ``MLP`` — 2-layer MLP used for proprio / projection heads.
...
class FeedForward(nn.Module):
    """Pre-norm feed-forward network used in transformer blocks."""
...
class Transformer(nn.Module):
    """Transformer stack supporting plain or AdaLN-zero blocks."""
...
class Embedder(nn.Module):
    """Smoothed 1D-conv → 2-layer MLP embedder used for actions."""
...
class MLP(nn.Module):
    """Simple 2-layer MLP with optional normalization and activation."""
...
class ARPredictor(nn.Module):
    ...
    Wraps a :class:`Transformer` built out of :class:`ConditionalBlock`
    layers so conditioning (action embedding) is injected via AdaLN.
    """
